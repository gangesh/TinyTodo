...
var mytinytodo = window.mytinytodo = _mtt = {
	...
	init: function(options)
	{
		...
		$("#users_form").live('submit', function() {
			saveSettings(this);
			return false;
		});
...
function editTask(id)
{
 ...
 document.getElementById('taskid_comment').value=item.id;
   var pas_tid ='';
   pas_tid = item.id;
	...
	toggleEditAllTags(0);
	showEditForm();
$("#comments").delay(800).load("comment_list.php", {taskid:item.id});
...
function showSettings()
{
	...
	$('#page_settings').fadeIn(1000).load(_mtt.mttUrl+'settings.php?ajax=yes',null,function(){ 
		showhide($('#page_settings').addClass('mtt-page-settings'), $('#page_tasks'));		
		showhide($('#page_settings').addClass('mtt-page-settings'), $('#page_users'));		

_mtt.pageSet('settings','settings');
	})
	return false;
}
...
function showUsers()
{
	if(_mtt.pages.current.page == 'ajax' && _mtt.pages.current.pageClass == 'users') return false;
	$('#page_users').fadeIn(1000).load(_mtt.mttUrl+'users.php?ajax=yes',null,function(){ 
		showhide($('#page_users').addClass('mtt-page-users'), $('#page_tasks'));
		showhide($('#page_users').addClass('mtt-page-users'), $('#page_settings'));


		_mtt.pageSet('users','users');
	})
	return false;
}

function saveUsers(frm)
{
	if(!frm) return false;
	var params = { save:'ajax' };
	$(frm).find("input:text,input:password,input:checked,select").filter(":enabled").each(function() { params[this.name || '__'] = this.value; }); 
	$(frm).find(":submit").attr('disabled','disabled').blur();
	$.post(_mtt.mttUrl+'users.php', params, function(json){
		if(json.saved) {
			flashInfo(_mtt.lang.get('settingsSaved'));
			setTimeout('window.location.reload();', 1000);
		}
